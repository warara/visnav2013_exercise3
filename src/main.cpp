...
//#include <ar_pose/Tag.h>
//#include <ar_pose/Tags.h>
...
struct Ardrone_localizer {
 ...
 void tfCB(const tf::tfMessageConstPtr msg)
 {
	 ...
	 {
		 if(msg->transforms[i].header.frame_id == "/ardrone_base_bottomcam" &&
	 ...
	 if(lastPos95t-lastPos1t > -0.01 && lastPos95t-lastPos1t < 0.01)
	 {
		 ...
		 tf::Vector3 relPos = Ry * tf::Vector3(
				 lastPos1[0] - lastPos95[0],
				 lastPos1[1] - lastPos95[1],
				 0);
		 ...
		 printf("relative orientation: %f %f %f (avg: %f %f %f)\n",
				 relPos[0],
				 relPos[1],
				 lastPos95[2] - lastPos1[2],
				 sumRelPos[0]/numRelPos,
				 sumRelPos[1]/numRelPos,
				 sumRelPos[2]/numRelPos);
	 }
 }
 ...
 void navCB(const ardrone_autonomy::NavdataConstPtr& nav_msg)
 {
	...
	if((ros::Time::now()-lastPublished).toSec() > 0.05)
	{
		...
		twist.linear.x = kalman_filter.state[0];
		twist.linear.y = kalman_filter.state[1];
		twist.angular.z = kalman_filter.state[2];
		pub_pose.publish(twist);
 ...
 Ardrone_localizer(){
  ...
  pub_pose = nh_.advertise<geometry_msgs::Twist>("/ardrone/filtered_pose",10);
...
int main(int argc, char **argv)
{
 ...
 tag_pose_4x4_95.setOrigin(tf::Vector3(localizer.global_pose_4x4_95[0],
                                     localizer.global_pose_4x4_95[1],
                                     localizer.global_pose_4x4_95[2]));
 tag_pose_4x4_1.setOrigin(tf::Vector3(localizer.global_pose_4x4_1[0],
                                     localizer.global_pose_4x4_1[1],
                                     localizer.global_pose_4x4_1[2]));
